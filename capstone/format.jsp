...
<c:if test="${curpage < '1'}">
    ...
    <script type="text/javascript">
        alert('없는 페이지 입니다.'); 
        location.href="./format.jsp?page=1";
    </script>
</c:if>


<sql:query var="dbcheck" dataSource="jdbc/mydb">
    select EXISTS (select * from format) as success;
</sql:query>

<sql:query var="todaypost" dataSource="jdbc/mydb">
    select COUNT(postdate) as count from format 
...
<sql:query var="atotalpost" dataSource="jdbc/mydb">
    select COUNT(*) as count from format;
</sql:query>
...
<c:choose>
    <c:when test='${searchText!=null && searchType=="title"}'>
        <sql:query var="totalpost" dataSource="jdbc/mydb">
            select COUNT(*) as count from format where title like ?;
            <sql:param value='%${searchText}%'/>
        </sql:query>
    </c:when>
    <c:when test='${searchText!=null && searchType=="content"}'>
        <sql:query var="totalpost" dataSource="jdbc/mydb">
            select COUNT(*) as count from format where content like ?;
            <sql:param value='%${searchText}%'/>
        </sql:query>
    </c:when>
    <c:when test='${searchText!=null && searchType=="manager"}'>
        <sql:query var="totalpost" dataSource="jdbc/mydb">
            select COUNT(*) as count from format where manager like ?;
            <sql:param value='%${searchText}%'/>
        </sql:query>
    </c:when>
    <c:otherwise>
...
<c:choose>
    <c:when test='${searchText!=null && searchType=="title"}'>
        <sql:query var="rs" dataSource="jdbc/mydb">
            select no, type, title, manager, date_format(postdate,'%Y-%m-%d %H:%i:%s') as postdate,
                views, file1, file2, file3 from format where title like ?
    ...
    <c:when test='${searchText!=null && searchType=="content"}'>
        <sql:query var="rs" dataSource="jdbc/mydb">
            select no, type, title, manager, date_format(postdate,'%Y-%m-%d %H:%i:%s') as postdate,
                views, file1, file2, file3 from format where content like ?
                order by no DESC limit ?, 15;
            <sql:param value='%${searchText}%'/>
            <sql:param value='${forwardpost}'/>
        </sql:query>
    ...
    <c:when test='${searchText!=null && searchType=="manager"}'>
        <sql:query var="rs" dataSource="jdbc/mydb">
            select no, type, title, manager, date_format(postdate,'%Y-%m-%d %H:%i:%s') as postdate,
                views, file1, file2, file3 from format where manager like ?
                order by no DESC limit ?, 15;
            <sql:param value='%${searchText}%'/>
            <sql:param value='${forwardpost}'/>
        </sql:query>
    ...
    <c:otherwise>
        <sql:query var="rs" dataSource="jdbc/mydb">
            select no, type, title, manager, date_format(postdate,'%Y-%m-%d %H:%i:%s') as postdate,
                views, file1, file2, file3 from format
                order by no DESC limit ?, 15;
            <sql:param value="${forwardpost}"/>
        </sql:query>
    </c:otherwise>
...
<html>
<head>
    <link rel="stylesheet" type="text/css" href="./css/style_format.css" />
...
<body>
    ...
    <%-- 카테고리 --%>
            <div class="sub7_tab_menu" style="text-align: center;">
    	    	...
    	    	<a class="sub7_tab check" href="./format.jsp?page=1" style="display: inline-block;">서식</a>
    	    	<a class="sub7_tab" href="./outcome.jsp?page=1" style="display: inline-block;">성과</a>
    	    </div>	

    <%-- 검색 --%>
            <div class="board-search">
                <form name="board3search" style="margin:0;"  action="./format.jsp?page=1&sc_column=${searchType}&sc_string=${searchText}" method="get">
    ...
    <%-- 게시글 목록 --%>
            <div class="responsive-table">
                <ul class="normal">
                    <li class="table-head">
                        ...
                        <div class="th th-cell02 w5">분류</div>
                        <div class="th th-cell03">제목</div>
                        <div class="th th-cell04 w10">작성자</div>
                        <div class="th th-cell05 w20">등록일</div>
                        <div class="th th-cell06 w10">조회수</div>
                            ...
                            <c:otherwise>
                                <c:forEach var="row" items="${rs.rows}">
                                    <li class="table-body-row">
                                        ...
                                        <div class="td td-cell02 w5">${row.type}</div>
                                        <div class="td td-cell03"><a href="./post.jsp?db=format&no=${row.no}" class="ntsviewBtn">${row.title}</a></div>
                                        <div class="td td-cell04 w10">${row.manager}</div>
                                        <div class="td td-cell05 w20">${row.postdate}</div>
                                        <div class="td td-cell06 w10">
                                            <c:choose>
            ...
            <div class="board-btn">
                <c:if test="${admin==true}">
                    <a href="./postwrite_fotice.jsp?db=format">
                            ...
                            <div class="pagePrev">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <span class="pageLeft">
                                            <a href='./format.jsp?page=${curpage-1}&sc_column=${searchType}&sc_string=${searchText}'>
                                                <span style="font-size:11px; color:#999999;">
                                                    <i class="fa fa-angle-left">
                                                    </i>
                                                </span>
                                ...
                                <c:forEach var="i" begin="1" end="${totalPage}">
                                    <c:choose>
                                        <c:when test="${curpage==i}">
                                            <li class="on" style="color:white"><a href='./format.jsp?page=${i}&sc_column=${searchType}&sc_string=${searchText}'>${i}
                                            </a></li>
                                        </c:when>
                                        <c:otherwise>
                                            <li style="color:white"><a href='./format.jsp?page=${i}&sc_column=${searchType}&sc_string=${searchText}'>${i}
                                            </a></li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                        ...
                        <div class="page">
                            <div class="pagePrev">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <span class="pageLeft">
                                            <a href='./format.jsp?page=${curpage-1}'>
                                                <span style="font-size:11px; color:#999999;">
                                                    <i class="fa fa-angle-left">
                                                    </i>
                                                </span>
                            ...
                            <ul>
                                <c:forEach var="i" begin="1" end="${totalPage}">
                                    <c:choose>
                                        <c:when test="${curpage==i}">
                                            <li class="on" style="color:white"><a href='./format.jsp?page=${i}'>${i}
                                            </a></li>
                                        </c:when>
                                        <c:otherwise>
                                            <li style="color:white"><a href='./format.jsp?page=${i}'>${i}
                                            </a></li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
