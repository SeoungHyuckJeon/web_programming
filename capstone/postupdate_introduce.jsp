			 ...
			 "utf-8", new DefaultFileRenamePolicy());
              ...
              if(check==1) { %>
                     <sql:update dataSource="jdbc/mydb" var="deleteTable">
                            UPDATE introduce SET file1=NULL where no = <%=no%>;
                     </sql:update>  
 <%           }
              if(check2==1) { %>
                     <sql:update dataSource="jdbc/mydb" var="deleteTable">
                            UPDATE introduce SET file2=NULL where no = <%=no%>;
                     </sql:update>  
 <%           }
              if(check3==1) { %>
                     <sql:update dataSource="jdbc/mydb" var="deleteTable">
                            UPDATE introduce SET file3=NULL where no = <%=no%>;
                     </sql:update>  
 <%           }
...
%>   
              <sql:update dataSource="jdbc/mydb" var="updatedTable">
	              UPDATE notice SET type=?, eventname=?, capacity=?, receptionstart=?, receptionend=?,
                            progressstart=?, progressend=?, manager=?, content=? where no = ?;
                            <sql:param value="<%=fom%>" />
...
<%            if(file1 != null) { %>
                     <sql:update dataSource="jdbc/mydb" var="updatedTable">
                            UPDATE introduce SET file1=? where no = ?;
...
<%            } 
              if(file2 != null) { %>
                     <sql:update dataSource="jdbc/mydb" var="updatedTable">
                            UPDATE introduce SET file2=? where no = ?;
                                   <sql:param value="<%=file2%>" />
                                   <sql:param value="<%=no%>" />
                     </sql:update> 
<%            } if(file3 != null) { %>
                     <sql:update dataSource="jdbc/mydb" var="updatedTable">
                            UPDATE introduce SET file3=? where no = ?;
